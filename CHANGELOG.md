...
- fix(versioning): read a bare current version back into a v-prefixed template (#1200)
...
- fix(formats): keep the padding a txt selector write used to eat (#1190)
...
- feat(fuzz): fuzz the config, format, commit and version parsers (#1182)

## [7.25.3] - 2026-09-23

### Bug Fixes

- fix(release): drop the stray padding from the unreachable-forge error (#1179)
...
- fix(validate): read a versioned file's selector instead of the whole file (#1171)

## [7.25.0] - 2026-09-22

### Features

- feat(action): sign the release tag with a configured SSH key (#1168)
...
- fix(forge): do not follow redirects when probing a host's forge (#1158)
...
- fix(doctor): report the token the push actually uses, and say the Gitea fallback is push-only (#1155)
...
- refactor: share the URL authority delimiters between the three host parsers (#1151)

## [7.24.1] - 2026-09-22

### Bug Fixes

- fix(auth): end the extraheader key's host at a backslash, like git (#1149)
...
- perf(ci): cut the PR critical path from 4m20 to about 2m35 (#1032)

## [7.19.0] - 2026-09-04

### Features

- feat(config): let a package override or switch off buildMetadata (#1028)

## [7.18.0] - 2026-09-04

### Features

- feat(docs): document buildMetadata in the workspace reference (#1026)
- feat(config): add buildMetadata, a command whose output is stamped after the plus (#1022)
